...
class orddd_class_view_deliveries {
            ...
            if ( $_REQUEST[ 'event_type' ] == 'product' ) {
                ...
                if( isset( $_REQUEST[ 'event_product_id' ] ) && $_REQUEST[ 'event_product_id' ] != "" ) {
                    ...
                    foreach ( $order_items as $item ) {
                        ...
                        $product_name .= '<a href="'.get_edit_post_link( $data['product_id']) .'" target="_blank">'. $item['name']. " x" . $item['quantity'] . '</a><br>';
                        ...
                        if( isset( $data[ 'variation_id' ] ) && $data[ 'variation_id' ] != 0 ) {
                            ...
                            if( is_array( $variation_data ) && count( $variation_data ) > 2 ) {
                                ...
                                foreach( $meta_data as $mkey => $mvalue ) {
                                    ...
                                    if( isset( $mvalue->display_key ) ) {
                                        $product_name .= '<strong>'.$mvalue->display_key. '</strong>' . ": " . force_balance_tags($mvalue->value) .'<br>';
                                    }
                                }
                            } 
                        }
    ...
    public function orddd_data_export() {    
        ...
        if ( isset( $_GET['download'] ) && ( $_GET['download'] == 'orddd_data.csv' ) && isset( $_GET[ 'page' ] ) && $_GET[ 'page' ] = 'orddd_view_orders' ) {
            ...
            echo $csv;
            exit;
        } else if( isset( $_GET[ 'download' ] ) && ( $_GET[ 'download' ] == 'orddd_data.print' ) && isset( $_GET[ 'page' ] ) && $_GET[ 'page' ] = 'orddd_view_orders' ) {
            $report  = self::orddd_generate_data();
    ...
    function orddd_generate_data() {
        ...
        foreach( $results as $rkey => $rval ) {           
            ...
            foreach ( $order_items as $item ) {
                ...
                array_push( $products, array( 'product' => $product_name, 'quantity' => $item[ 'quantity' ] ) );
