	    	...
	    	if( false == $lockout_days_str ) {
			    ...
			    if ( get_option( 'orddd_enable_time_slot' ) == 'on' ) {
		            ...
		            foreach ( $date as $k => $v ) {
						foreach( $v as $timeslot => $lockout ) {
							...
							if ( is_array( $delivery_days ) && count( $delivery_days ) > 0 ) {
								...
								if ( isset( $delivery_days[ 'orddd_weekday_' . $weekday ][$timeslot] ) ) {
									if( $v[$timeslot] >= $delivery_days[ 'orddd_weekday_' . $weekday ][$timeslot] && '' != $delivery_days[ 'orddd_weekday_' . $weekday ][$timeslot]  && '0' != $delivery_days[ 'orddd_weekday_' . $weekday ][$timeslot] && 'yes' != $blank_lockout_time_slot) {
										if( isset( $lockout_arr[$k] ) ) {
											array_push( $lockout_arr[$k], $timeslot );
										}else {
											$lockout_arr[$k] = array( $timeslot );
		            ...
		            foreach ( $lockout_arr as $k => $v ) {
						...
						if ( is_array( $delivery_days ) && count( $delivery_days ) > 0 ) {
							$weekday = date( 'w', strtotime( $k ) );
							if ( isset( $delivery_days[ 'orddd_weekday_' . $weekday ] )  && is_array( $lockout_arr[$k] ) && count( $lockout_arr[$k] ) >= count( $delivery_days[ 'orddd_weekday_' . $weekday ] ) ) {
								$lockout = explode( "-", $k );
								$lockout_days_str .= '"' . intval( $lockout[ 1 ] ) . '-' . intval( $lockout[ 0 ] ) . '-' . $lockout[ 2 ] . '",';
							}
						}
					...
					if ( get_option( 'orddd_lockout_date_after_orders' ) > 0 ) {
						...
						foreach ( $lockout_days_arr as $k => $v ) {
							$delivery_dates = json_decode(get_option( 'orddd_delivery_dates' ), true);
	...
	public static function orddd_get_custom_lockout_days( $shipping_settings ) {
		...
		if( isset( $shipping_settings[ 'time_slots' ] ) && $shipping_settings[ 'time_slots' ] != '' ) {
            			...
            			if ( $timeslot_values[ 'delivery_days_selected' ] == 'weekdays' ) {
            				foreach( $timeslot_values[ 'selected_days' ] as $dkey => $dval ) {
            					if( $timeslot_values[ 'lockout' ] != "" && $timeslot_values[ 'lockout' ] != "0" ) {
            						...
            						if( 'all' == $dval ) {
            							$all_lockout += get_option( 'orddd_global_lockout_time_slots' );
            						} else {
            							if ( isset( $delivery_days[$dval][$timeslot] ) ) {
            								$previous_lockout =  $delivery_days[ $dval ][$timeslot] + get_option( 'orddd_global_lockout_time_slots' );
            								$delivery_days[ $dval ][$timeslot] = $previous_lockout;
            							} else {
            				...
            				foreach( $timeslot_values[ 'selected_days' ] as $dkey => $dval ){
            					if ( isset( $specific_dates[ $dval ][$timeslot] ) ) {
            						if( $timeslot_values[ 'lockout' ] != "" && $timeslot_values[ 'lockout' ] != "0" ) {
            							...
            							$previous_lockout =  $specific_dates[ $dval ][$timeslot] + get_option( 'orddd_global_lockout_time_slots' );
            						} else {
            							$blank_lockout_time_slot = "yes";
            						}
            						$specific_dates[ $dval ][$timeslot] = $previous_lockout;
			...
			if ( isset( $timeslot_field_mandatory ) && "" == $timeslot_field_mandatory ) {
				...
				if ( isset( $shipping_settings[ 'date_lockout' ] ) && $shipping_settings[ 'date_lockout' ] != '0' &&  $shipping_settings[ 'date_lockout' ] != '' ) {
	                ...
	                if( isset( $shipping_settings[ 'orddd_lockout_date' ] ) ) {
	                    $lockout_date = $shipping_settings[ 'orddd_lockout_date' ];
            ...
            foreach ( $date as $k => $v ) {
				foreach( $v as $timeslot => $lockout ) {
					...
					if ( is_array( $delivery_days ) && count( $delivery_days ) > 0 && !isset( $specific_dates[ $specific_date ] ) ) {
						$weekday = date( 'w', strtotime( $k ) );
						if ( isset( $delivery_days[ 'orddd_weekday_' . $weekday . '_custom_setting' ][$timeslot] ) ) {
							if( $lockout >= $delivery_days[ 'orddd_weekday_' . $weekday . '_custom_setting' ][$timeslot] && '' != $delivery_days[ 'orddd_weekday_' . $weekday . '_custom_setting' ][$timeslot]  && '0' != $delivery_days[ 'orddd_weekday_' . $weekday . '_custom_setting' ][$timeslot] && 'yes' != $blank_lockout_time_slot) {
								if( isset( $lockout_arr[$k] ) ) {
									array_push( $lockout_arr[$k], $timeslot );
								}else {
									$lockout_arr[$k] = array( $timeslot );
            ...
            foreach ( $lockout_arr as $k => $v ) {
				...
				if ( isset( $delivery_days[ 'orddd_weekday_' . $weekday . '_custom_setting' ] ) && is_array( $lockout_arr[$k] ) && count( $lockout_arr[$k] ) >= count( $delivery_days[ 'orddd_weekday_' . $weekday . '_custom_setting' ] )  ) {
					$date_to_block = explode( "-", $k );
            		$lockout_days_str .= '"' . intval( $date_to_block[ 1 ] ) . '-' . intval( $date_to_block[ 0 ] ) . '-' . $date_to_block[ 2 ] . '",';
				}
            }
